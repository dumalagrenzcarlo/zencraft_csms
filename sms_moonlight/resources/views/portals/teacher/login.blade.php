...
<head>
    ...
    <title>{{ \App\Support\SchoolBranding::name() }} Teacher Login</title>
    ...
    <main class="flex min-h-screen items-center justify-center px-5 py-10">
        <section class="w-full max-w-md rounded-[2rem] border bg-[var(--portal-surface)] p-6 shadow-2xl sm:p-8" style="border-color: color-mix(in srgb, var(--portal-primary) 28%, #ffffff); box-shadow: 0 24px 70px color-mix(in srgb, var(--portal-primary) 16%, transparent);">
            <div class="text-center">
                ...
                <p class="mt-5 text-xs font-bold uppercase tracking-[0.22em]" style="color: var(--portal-primary);">Teacher Portal</p>
                <h1 class="mt-2 text-2xl font-bold tracking-tight" style="color: var(--portal-text);">{{ $schoolName }}</h1>
                <p class="mt-2 text-sm text-slate-500">Sign in to manage your classes and student records.</p>
            </div>
            ...
            <form class="mt-6 space-y-4" method="POST" action="{{ route('teacher.login.submit') }}">
                @csrf
                <div>
                    <label class="mb-1.5 block text-sm font-semibold" style="color: var(--portal-text);" for="username">Username</label>
                    <input class="w-full rounded-2xl border bg-white px-4 py-3 text-sm outline-none transition focus:ring-4" style="border-color: color-mix(in srgb, var(--portal-primary) 22%, #ffffff); --tw-ring-color: color-mix(in srgb, var(--portal-primary) 16%, transparent);" id="username" name="username" type="text" value="{{ old('username') }}" required autofocus>
                </div>
                <div>
