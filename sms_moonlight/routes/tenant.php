...
$registerPortalRoutes = static function (string $portal, \Closure $routes) use ($isProduction): void {
    $domain = config("school_portal.domains.{$portal}");
    ...
    if ($isProduction && filled($domain)) {
        Route::domain($domain)->group($routes);

        return;
    }

    Route::prefix($portal)->group($routes);
};
...
Route::middleware('portal.detect')->get('/', function () use ($portalForHost) {
    ...
    if ($portal === 'student') {
        ...
        $hasAccess = \App\Models\StudentAccess::where('user_id', $user->id)
            ->activeForPortal()
            ->exists();
    ...
    if ($portal === 'teacher') {
        ...
        $isTeacher = \App\Models\Adviser::where('user_id', $user->id)
            ->whereIn('staff_type', [\App\Models\Adviser::TYPE_TEACHER, \App\Models\Adviser::TYPE_INSTRUCTOR])
            ->exists();
...
Route::get('/students/template', function () {
    ...
    return response()->download(
        resource_path(
            'templates/import-student-template.xlsx'
        ),
